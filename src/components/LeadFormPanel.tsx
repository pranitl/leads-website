...
export default function LeadFormPanel({
  ...
  return (
    <div class={`card-rustic w-full max-w-md text-brown-dark ${className ?? ''}`} id="lead-form">
      <div class="card-rustic-header flex items-center gap-2">
        <svg viewBox="0 0 24 24" class="h-5 w-5 fill-current" aria-hidden="true">
          <path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-7 14H6v-2h6v2zm4-4H6v-2h10v2zm0-4H6V7h10v2z"/>
        </svg>
        <span>Get Your Free Project Estimate</span>
      </div>
      <div class="card-rustic-body">
        <p class="text-xs font-semibold uppercase tracking-[0.25em] text-brown-forest">Step {step} of 2</p>
        <h2 class="mt-2 font-heading text-2xl font-semibold text-brown-dark">
          {step === 1 ? 'Find vetted pros near you' : `Great! We've found 3 pros in ${displayZip || 'your area'}.`}
        </h2>
        <p class="mt-2 text-sm text-brown-body">
          {step === 1
      ...
      {step === 1 ? (
        <form class="space-y-4" onSubmit={startQualification} noValidate>
          <div class="space-y-1">
            <label class="text-sm font-medium text-brown-body" htmlFor={`${formId}-service`}>
              Service needed
            </label>
            <select
              id={`${formId}-service`}
          ...
          <div class="space-y-1">
            <label class="text-sm font-medium text-brown-body" htmlFor={`${formId}-zip`}>
              ZIP code
            </label>
            <input
              id={`${formId}-zip`}
        ...
        <div class="space-y-4">
          <div class="flex items-center justify-between rounded-lg bg-cream px-3 py-2 text-xs text-brown-body">
            <span>
              {selectedService ? selectedService.name : 'Service'} · ZIP {displayZip}
            </span>
            <button
              type="button"
              class="font-semibold text-oxide hover:text-oxide-hover"
              onClick={() => {
                setStep(1);
                setStatus('idle');
                setErrors(createEmptyErrors());
          ...
          {status === 'success' ? (
            ...
            >
              <div class="space-y-1">
                <label class="text-sm font-medium text-brown-body" htmlFor={`${formId}-name`}>
                  Full name
                </label>
                <input
                  id={`${formId}-name`}
              ...
              <div class="grid gap-4 sm:grid-cols-2">
                <div class="space-y-1">
                  <label class="text-sm font-medium text-brown-body" htmlFor={`${formId}-email`}>
                    Email
                  </label>
                  <input
                    id={`${formId}-email`}
                    ...
                    required
                  />
                  <p class="text-xs text-brown-muted">We’ll email your match confirmation here.</p>
                  {errors.email && <p class="text-xs text-danger">{errors.email}</p>}
                </div>
                <div class="space-y-1">
                  <label class="text-sm font-medium text-brown-body" htmlFor={`${formId}-phone`}>
                    Phone
                  </label>
                  <input
                    id={`${formId}-phone`}
                    ...
                    required
                  />
                  <p class="text-xs text-brown-muted">So matched pros can send you a quote. We never spam.</p>
                  {errors.phone && <p class="text-xs text-danger">{errors.phone}</p>}
                </div>
              </div>

              <div class="space-y-1">
                <label class="text-sm font-medium text-brown-body" htmlFor={`${formId}-address`}>
                  Full address
                </label>
                <input
                  id={`${formId}-address`}
              ...
              <fieldset class="space-y-2">
                ...
                <div class="grid gap-2 sm:grid-cols-3">
                  {[
                    ...
                    <label
                      key={option.value}
                      class={`rounded-lg border px-3 py-2 text-xs font-semibold uppercase tracking-wide transition ${values.timeline === option.value ? 'border-brown-forest bg-cream text-brown-forest' : 'border-brown-forest/30 text-brown-muted hover:border-brown-forest'}`}
                    >
                      <input
                        type="radio"
                        name="timeline"
              ...
              <fieldset class="space-y-2">
                <legend class="text-sm font-medium text-brown-body">What is your estimated budget?</legend>
                <div class="grid gap-2 sm:grid-cols-2">
                  {[
                    ...
                    <label
                      key={option.value}
                      class={`rounded-lg border px-3 py-2 text-xs font-semibold uppercase tracking-wide transition ${values.budget === option.value ? 'border-brown-forest bg-cream text-brown-forest' : 'border-brown-forest/30 text-brown-muted hover:border-brown-forest'}`}
                    >
                      <input
                        type="radio"
                        name="budget"
              ...
              <div class="space-y-1">
                <label class="text-sm font-medium text-brown-body" htmlFor={`${formId}-details`}>
              ...
              <div class="flex items-start gap-3">
                ...
                />
                <label class="text-xs text-brown-body" htmlFor={`${formId}-consent`}>
              ...
              <div class="flex items-center gap-2 rounded-lg border-2 border-brown-forest/20 bg-cream px-3 py-2 text-xs text-brown-body">
                <svg viewBox="0 0 20 20" aria-hidden="true" class="h-4 w-4 fill-current text-brown-forest">
